...
enum APIEndpoinstEnum {
  ...
  var url: String {
    ...
    case .searchWithFilter(let query, let page, let order, let orientation):
          var url = "https://api.unsplash.com/search/photos?query=\(query)&page=\(page)&per_page=20&client_id=akXKH70mCBd9NsLvhnZEVwz8wVN0urhfmB2fKi7_ouU&order_by=\(order.rawValue)"
          if orientation != .all {
